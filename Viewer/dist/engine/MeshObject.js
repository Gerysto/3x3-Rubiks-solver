...
export class MeshObject {
    constructor() {
        ...
        this.normals = [];
        this.faces = [];
        this.boundingBox = new Box(new Vec3(1e100, 1e100, 1e100), // Min
    ...
    readObj(obj_url) {
        return __awaiter(this, void 0, void 0, function* () {
            const response = yield fetch(obj_url);
            if (!response.ok) {
            ...
            for (let i = 0; i < lines.length; ++i) {
                ...
                switch (c) {
                    ...
                    case 'f':
                        ...
                        if (first == -1)
                            this.parseV(arr, curr_mtl);
                        let second = arr[1].indexOf("/", first + 1);
                        if (second == -1)
                            this.parseVT(arr, curr_mtl);
                        if (second == first + 1)
                            this.parseVN(arr, curr_mtl);
                        else
                            this.parseVTN(arr, curr_mtl);
                        break;
                    case 'm': // Check for "mtllib"
                        arr = line.split(" ");
                        if (arr[0] == "mtllib") {
                            let url_mtl = arr[1];
                            yield this.loadMaterials(url_mtl, obj_url);
                        }
                        else {
                            // Not yet supported : (
                        }
    ...
    loadMaterials(mtl_url, obj_url) {
        return __awaiter(this, void 0, void 0, function* () {
            // Fetch .mtl file
            const dir = obj_url.substring(0, obj_url.lastIndexOf("/") + 1);
            //console.log(dir);
            const response = yield fetch(dir + mtl_url);
            if (!response.ok) {
